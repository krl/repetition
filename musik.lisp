...
(defparameter kick 60)
(defparameter snare 62)
(defparameter rim 63)
(defparameter hic 65)
(defparameter hio 66)
(defparameter crash 68)


(defun play-on-channel (channel data)
  (let ((time 2))
    (loop for e in data do
	 (loop for h in (rest e) do
	      (send-data (make-note channel h 100 (beat time))))
	 (incf time (first e)))))
...
;(setq chords (list (append '(1) (tonalize (chord *scale*) *key*))))
